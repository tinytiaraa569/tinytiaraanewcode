...
const ContactBanner = require('../model/contactbanner'); // Import the Banner model
const fs = require('fs');
...
router.post('/contact-create-banners', catchAsyncErrors(async (req, res, next) => {
    const { title, link, images } = req.body;
    ...
    const processBase64Image = (imagePath, destinationArray) => { 
        if (typeof imagePath === "string" && imagePath.startsWith("data:")) {
            ...
            const extension = mimeType.split('/')[1];
            const uniqueId = generateRandomString(20);
            const localImagePath = path.join(__dirname, '../uploads/images/banners/contact', `${uniqueId}.${extension}`);
    
            fs.writeFileSync(localImagePath, Buffer.from(base64Data, 'base64'));
            destinationArray.push({
                public_id: `banners/${uniqueId}`,
                url: `/uploads/images/banners/contact/${uniqueId}.${extension}`
            });
        } else {
            destinationArray.push({ url: imagePath });
        }
...
router.get('/get-allcontactbanners', catchAsyncErrors(async (req, res, next) => {
...
// // Get a banner by ID
router.get('/get-contactbanner/:id', catchAsyncErrors(async (req, res, next) => {
...
router.put('/update-contactbanner/:id', catchAsyncErrors(async (req, res, next) => {
    const { title, link, images } = req.body;
...
router.delete('/delete-contactbanner/:id', catchAsyncErrors(async (req, res, next) => {
    const { id } = req.params;

    // Find and delete the banner
    const bannerToDelete = await ContactBanner.findByIdAndDelete(id);
    ...
    const remainingBanners = await ContactBanner.find().sort({ order: 1 }); // Sort banners by current order
...
//     }
// }));
router.post('/delete-multiple-contactbanners', catchAsyncErrors(async (req, res, next) => {
    const { ids } = req.body; // Expecting an array of IDs in the request body
    ...
    try {
        // Delete all banners with the provided IDs
        const deleteResult = await ContactBanner.deleteMany({ _id: { $in: ids } });
        ...
        const remainingBanners = await ContactBanner.find().sort({ order: 1 }); // Sort by current order
...
router.post('/update-contactbanner-order', catchAsyncErrors(async (req, res, next) => {
    ...
    try {
        for (const { id, order } of orderedBanners) {
            await ContactBanner.findByIdAndUpdate(id, { order });
        }
